...
exports.signup = async (req, res) => {
  try {
    ...
    const newUser = await User.create({
      ...
      role,
      subscriptionExpires,
      hasStock: hasStock === true || hasStock === "true", // ضمان تحويل القيمة
...
exports.updateUser = async (req, res) => {
  try {
    ...
    // 🟢 إصلاح: معالجة صريحة لحقل hasStock لضمان تحديثه
    if (req.body.hasStock !== undefined) {
      req.body.hasStock = Boolean(req.body.hasStock);
    }
...
exports.updateStaff = async (req, res) => {
  try {
    ...
    if (!staffMember) {
      return res
        .status(404)
        .json({ message: "الموظف غير موجود أو لا يتبع لك" });
    }
...
exports.updateStaff = async (req, res) => {
  try {
    ...
    if (!staffMember) {
      return res
        .status(404)
        .json({ message: "الموظف غير موجود أو لا يتبع لمطعمك" });
    }
...
exports.deleteStaff = async (req, res) => {
  try {
    ...
    if (!staffMember) {
      return res
        .status(404)
        .json({ status: "fail", message: "الموظف غير موجود أو لا يتبع لك" });
    }
